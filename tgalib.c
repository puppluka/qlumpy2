...
static byte cust_pal[QPALSIZE];
static byte c_last; 
static pixel_t last_px; 
static qboolean palette_initialized = false;

/* Fallback palette from Quake, put into the Public Domain by John Carmack */
static pixel_t pal_fallb[256] = {
...
static void InitPalette() {
    ...
    if (fPAL) {
        printf("Loaded custom palette.lmp\n");
        fread(cust_pal, 1, QPALSIZE, fPAL);
        fclose(fPAL);
    } else {
        printf("No palette.lmp found, using fallback palette\n");
        for (int p = 0; p < 256; p++) {
